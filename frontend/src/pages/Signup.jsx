...
const Signup = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      alert(`NEW RANGER RECRUITED: ${formData.codeName.toUpperCase()} (${formData.rangerColor.toUpperCase()} RANGER)`);
      setLoading(false);
    }, 2000);
  };
        ...
        <div className="flex max-w-6xl w-full bg-gray-900/90 backdrop-blur-xl rounded-3xl shadow-[0_0_80px_rgba(168,85,247,0.4)] border border-purple-500/50 overflow-hidden floating-card">
            ...
            <div className="hidden lg:flex w-5/12 relative overflow-hidden image-container group cursor-pointer">
                ...
                <div className="relative z-10 p-10 flex flex-col justify-between h-full">
                    <div className="border-l-4 border-cyan-400 pl-6">
                        <h2 className="text-5xl font-black text-white tracking-tighter drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] glitch" data-text="RECRUIT">
                            RECRUIT
                        </h2>
                    ...
                    <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl border border-cyan-500/30 transform group-hover:translate-x-2 transition-transform duration-300">
                        <p className="text-cyan-300 text-sm font-semibold tracking-widest mb-2">OBJECTIVE: ASSEMBLE</p>
                        <p className="text-gray-300 text-sm leading-relaxed">
                            "The grid is expanding. We need new guardians to protect the universe. Choose your color."
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full lg:w-7/12 p-10 md:p-12 relative">
                <div className="absolute top-0 right-0 p-6 opacity-30">
                     <svg xmlns="http://www.w3.org/2000/svg" className="w-20 h-20 text-cyan-500 animate-pulse" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-cyan-400/80 uppercase tracking-[0.2em] text-sm">Create Your Legacy</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label className="block text-gray-400 text-xs uppercase tracking-widest mb-3">Select Power Source</label>
                        <div className="flex gap-4 justify-start">
                            {colors.map((color) => (
                                <div 
                                    key={color.name}
                                    onClick={() => handleColorSelect(color.name)}
                                    className={`w-10 h-10 rounded-full cursor-pointer color-orb ${formData.rangerColor === color.name ? 'selected' : ''}`}
                    ...
                    <div className="pt-2">
                        ...
                        >
                            {loading ? 'GENERATING MORPHER...' : 'INITIATE TRANSFORMATION'}
                        </button>
                    </div>

                    <div className="mt-6 text-center">
                        <p className="text-gray-500 text-sm">Already active?</p>
                        <button type="button" className="text-cyan-400 hover:text-white transition-colors text-sm uppercase tracking-widest border-b border-transparent hover:border-cyan-500 pb-1 mt-1">
                            Return to Login Hub
                        </button>
                    </div>
                </form>
